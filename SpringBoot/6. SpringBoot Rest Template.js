...
Method              HTTP       설명
1. getForObject     GET        GET 형식으로 요청하여 객체로 결과를 반환받음.
2. getForEntity     GET        GET 형식으로 요청하여 ResponseEntity로 결과를 반환받음
3. postForObject    POST       POST 형식으로 요청하여 객체로 결과를 반환받음
4. postForEntity    POST       POST 형식으로 요청하여 ResponseEntity로 결과를 반환받음
5. delete           DELETE     DLETE 형식으로 요청
6. put              PUT        PUT 형식으로 요청
7. patchForObject   PATCH      PATCH 형식으로 요청
8. exchange         any        HTTP 헤더를 생성하여 추가할 수 있고, "어떤 형식"에서도 사용 가능
